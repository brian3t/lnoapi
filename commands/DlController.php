...
//        $crawler = $client->request('GET', SDREADER, ['start_date' => '2018-07-04', 'end_date' => '2018-07-04']);
        ...
        $crawler->filter('table.event_list tr')->each(function ($event_and_venue) use (&$records){
            ...
            if (!$venue_exist instanceof Venue) {
                $venue = new Venue();
                $venue->setAttributes(['name' => $venue_name, 'system_note' => $venue_href]);
                $venue->save();
                $venue_id = $venue->id;
            } else {
            ...
            if (!$event_exist instanceof Event) {
                $event = new Event();
                $event->setAttributes(['created_by' => $this->SCRAPER_ID, 'name' => $event_name, 'system_note' => $event_href]);
